...
namespace VyrokovaLogikaPrace
{
    ...
    public class TreeProof
    {
        ...
        public TreeProof(Node tree, int truthValue = 0)
        {
            ...
            if (triedToAssignLiterals.Count == 0)
            {
                if (tree.Left != null)
                {
                    ...
                    if (values.Count > 1 && tree.UsedCombinations == null)
                    {
                        ...
                        tree.TreeAllOptions = tree.UsedCombinations.Count;
                        tree.combinationUsedCounter += 1;
                        tr = Node.DeepCopy(GetToRoot(tree));
                        moreOptions.Add(tr);
                    }
                    ...
                    if (wasSomethingChanged)
                    {
                        ...
                        {
                            steps.Add("Zkusíme přidat jako "+tree.combinationUsedCounter+". ze " + tree.TreeAllOptions +" možností na levou stranu " + tree.Left.TruthValue);
                        }
                        Node tempTree = new Node(0);
                        tempTree = Node.DeepCopy(GetToRoot(tree));
                        trees.Add(tempTree);
